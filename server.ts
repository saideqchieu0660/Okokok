...
async function executeGenerateContentRoundRobin(contents: any, config: any = {}): Promise<string> {
  ...
  for (const provider of activeProviders) {
    try {
      ...
      if (provider === "groq") {
        const numKeys = Math.min(groqKeyStates.length, 5);
        for (let kIdx = 0; kIdx < numKeys; kIdx++) {
          let currentGroqState: KeyState | null = null;
          try {
            ...
            if (text) {
              addGroqRotationLog({ toKeyIndex: state.index, reason: `Execution successful` });
              return text;
            }
          } catch (err: any) {
            finalError = err;
            if (currentGroqState) handleGroqError(currentGroqState, err);
          }
        }
      }
...
${text}`;
      ...
      let responseText = await executeGenerateContentRoundRobin(prompt);
...
NHIỆM VỤ TỐI THƯỢNG:
1. TUYỆT ĐỐI KHÔNG THAY ĐỔI, KHÔNG THÊM, KHÔNG XÓA, KHÔNG DIỄN GIẢI lại bất kỳ từ ngữ nào. Toàn bộ dữ liệu gốc PHẢI được giữ nguyên vẹn 100%, CHỈ CHÈN THÊM ký tự xuống dòng (\n) và khoảng trắng (space).
2. Tự động chèn các dấu xuống dòng (\\n) và khoảng cách hợp lý để làm cho nội dung thông thoáng, cực kỳ dễ đọc, KHÔNG BỊ DÍNH CHÙM.
3. ĐẶC BIỆT CHÚ Ý các câu hỏi trắc nghiệm có các lựa chọn đáp án (A., B., C., D. hoặc A), B), C), D) hoặc 1., 2., 3., 4.) dính liền vào câu hỏi hoặc dính liền nhau: TÁCH RIÊNG MỖI LỰA CHỌN RA MỘT DÒNG RIÊNG BIỆT (Ví dụ: Dòng câu hỏi chính ở trên, sau đó xuống dòng cho A., B., C., D. ở phía dưới).
4. Đối với nội dung giải thích/mặt sau: Tách các ý thành dòng gạch đầu dòng (-) hoặc các đoạn văn ngắn dễ theo dõi.
