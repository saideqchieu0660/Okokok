...
import { 
  ArrowLeft, Volume2, VolumeX, Sparkles, Check, X, BellPlus, Edit3, Settings, AlertCircle, Zap, Sliders,
  Copy, CheckCheck, FileText, Plus, Timer as TimerIcon, Palette, BarChart2, Wrench
} from 'lucide-react';
...
export const VibeFlashcardActiveView: React.FC<VibeFlashcardActiveViewProps> = React.memo(({
  ...
  const handleFormatAI = async (e: React.MouseEvent) => {
    ...
    try {
      const res = await fetch("/api/automation/format-card", {
        method: "POST",
      ...
      console.error("Format AI Error:", err);
