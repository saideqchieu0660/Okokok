...
export function syncLocalUserDecks() {
  ...
  if (!currentUser) {
      ...
      const systemDecks = [
        "deck_1", "deck_phil_2", "deck_math_1", "deck_math_2", "deck_physics_1", "deck_physics_2", "deck_test_ui", "deck_formatting_test", "daily-quest", "remind-later-deck"
      ];
...
export function saveLocalUserDecks() {
  ...
  const localUserDecksKey = `local_user_decks_${userId}`;
  const systemDecks = [
    "deck_1", "deck_phil_2", "deck_math_1", "deck_math_2", "deck_physics_1", "deck_physics_2", "deck_test_ui", "deck_formatting_test"
  ];
     ...
     if (currentUser && currentUser.id) {
         ...
         import('../vibe-sandbox/sync/VibeSyncEngine').then(({ VibeSyncEngine }) => {
            VibeSyncEngine.enqueueChange({
               ...
               payload: { cardId: card.id, uid: currentUser!.id, ...payload }
            });
